...
This folder contains PowerShell scripts to retrieve and export Microsoft Copilot interaction audit logs from Microsoft 365 using the Microsoft Graph API. The CSVs produced by these scripts feed the AI Business Value Dashboard PBIT template at the root of this repo.

> **Attribution:** These scripts are taken from the [Microsoft AI-in-One Dashboard](https://github.com/microsoft/AI-in-One-Dashboard/tree/main/scripts) repository (MIT License, © Microsoft Corporation). No material changes to script logic — they produce the same CSV outputs expected by the dashboard template.
