...
|---|---|---|
| `ValueLens - Fabric (Power Query, pre-2307).pbit` | The previous template. It did all the audit-log JSON parsing, list-explode and joins **inside Power Query**, on every refresh. | Replaced by the **2307 (Spark)** build. That heavy shaping now runs once in the `Copilot_Interactions_Curate` notebook, so the current `../ValueLens - Fabric.pbit` reads a ready-made table and refreshes in seconds instead of hours. See [`../README.md`](../README.md). |
